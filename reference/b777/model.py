...
class PerformanceModel:
    ...
    def __init__(self, settings: ModelSettings | None = None):
        from openap import Drag, FuelFlow, Thrust, prop

        if version("openap") != OPENAP_VERSION:
            raise RuntimeError(f"Install openap=={OPENAP_VERSION} for reproducible results")
        self.settings = settings or ModelSettings()
        self.settings.validate()
        self.aircraft = prop.aircraft(AIRCRAFT)
        self.engine = prop.engine(ENGINE)
        self.polar = Drag(AIRCRAFT).polar["clean"]
        self.wing_area_m2 = float(self.aircraft["wing"]["area"])
        self.fuel_model = FuelFlow(AIRCRAFT, eng=ENGINE)
        self.thrust_model = Thrust(AIRCRAFT, eng=ENGINE)
